...
class Binarize_Hopfield:
	def __init__(self, raw_data, m, p, precision, p_w=1, binarize_threshold=None):
		''' Neurons takes binary value 0 or 1.
        args:
           N (int): dimension of the net
           memories (nparray): memory patterns with dimension [number of examples, N]
		...
		if binarize_threshold == 'avg':
			binarize_threshold = self.W.mean()
		elif binarize_threshold == 'median':
			binarize_threshold = self.W.median()
		else:
			binarize_threshold = 0
		
		self.W = (self.W >= binarize_threshold).to(precision) / m
		self.p_w = p_w
		if p_w < 1:
			self.W = self.W * (torch.rand(m, m).cuda() < p_w)
